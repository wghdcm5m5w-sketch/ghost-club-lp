...
- [ ] プロジェクトを開く:
  - **最小構成（本体のみ）**: 同梱の `app/TetsuLog.xcodeproj` をそのまま開く
  - **フル構成（本体+Widget+Watch+Tests）**: `brew install xcodegen && cd app && xcodegen generate`
    → 4ターゲットが揃った `TetsuLog.xcodeproj` が生成される（Capabilities は下記で付与）
