...
## STEP 3.5 ── スクリーンショット再撮影（必須）

`app/appstore-screenshots-6.5/` の7枚は**旧デザイン（クリーム国鉄調）で撮ったもの**。
現在のハイブリッド（近黒＋シアン＋硬券）とは別物なので、**実機/シミュレータで撮り直す**こと。

1. Xcode の実行先を **iPhone 16 Pro Max（6.9"）** シミュレータに
2. ⌘R → 各画面を開いて **⌘S**（シミュレータの File → Save Screen で 1320×2868 / 1290×2796 が保存される）
   - 図鑑（側面図カード）/ 記録（硬券一覧）/ 遭遇詳細（大判硬券）/ 地図（製図オーバーレイ）/ 統計（ゲージ）/ 設定（特別補充券Pro）から 6〜7枚
3. iPad 対応のため **iPad Pro 13"** でも数枚あると尚良
4. App Store Connect の対応サイズ枠にアップロード（6.9"/6.7" の枠は1セットでOK）

> シードデータ（52形式・1393編成・20撮影地）が入っているので、空っぽに見えず映える。
